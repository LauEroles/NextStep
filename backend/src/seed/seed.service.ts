...
export class SeedService {
  ...
  constructor(
    ...
    private readonly seniorityRepo: Repository<Seniority>,
  ...
  async runSeed() {
    ...
    if ((await this.userRepo.count()) === 0) {
      ...
      this.logger.log(
        `✅ Usuario Administrador creado con éxito. ID: ${savedAdmin.id}, role_id: ${savedAdmin.role.id}`,
      );
      this.logger.log(
        'Sus credenciales de administrador son las que definió en las variables de entorno DEFAULT_ADMIN_EMAIL y DEFAULT_ADMIN_PASSWORD.',
      );
    }
